...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Al terminar el ciclo, pasamos por todos los tablones y tenemos $S^{opt}$
    que es subsolución de una solución óptima. Sea $S*$ tal solución óptima.
    Si $S \gets S*$ entonces $S^{opt}$ es una solución óptima.
    Si no, $existe x \in S* \setminus S^{opt}$, Pero $x \in S y p(S*, x) \gets TRUE$,
    por lo tanto $p(S^{opt}, x) \gets TRUE y P(X, x) \gets TRUE$ para todo prefijo
    X de $S^{opt}$. Entonces, al momento de sacar $x$ de $S$ e intentar agregarlo
    a nuesta $S^{opt}$, deberíamos haberlo podido agregar, y no lo hicimos. Esto
    no puede pasar, luego no existe $x$, y $S* \gets S^{opt}$.
