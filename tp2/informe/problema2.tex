...
%\li   \Do
%\li       $\id{t} \gets \proc{crearTablero}(n)
%\li       $\proc{llenarTablero}(t)$
%\li       $\proc{agregar}(t, tableros)$
%      \End
%\li $\id{i} \gets 0$
%\li $\id{j} \gets 0$
%    $\id{min_i} \gets -1$
...
% \li $\id{casilleros} \gets matriz(n x n) inicializados en -1$
%
% \Procname{$\proc{llenarTablero}(tablero, casilleros)$}
% \li \While(!vacio?(casilleros))
% \li   \Do
%         posicion = casilleros.primero.pos
%         nivel = casilleros.primero.nivel
%         casilleros.sacarPrimero
%
%         if tablero.casilleros[posicion].pos == -1
%            continuar
%         end
%
%         tablero.casilleros[posicion].nivel = nivel
%
%         \For $posiblesMovimientos(posicion))$       ESTE FOR NO ESTA CLARO, LA "POSICION" NO CAMBIA
%           if noSalioDelTablero(posicion) y posicion.nivel != -1
%             posicion.nivel = nivel + 1
%             casilleros.agregar(posicion)
%           end
%       \End
%
